            ...
            <div class="content-body">
                ...
                <div class="tab-content">
                    <div class="tab-pane bg-white active">
                        <div class="py-1 px-2">
                            @include('backend.taxInvoice.bottom-header', ['activeMenu' => 'edit',])
                        </div>
                        <section id="widgets-Statistics">
                            <form action="{{route('tax-invoice-update')}}" method="POST" id="formSubmit" enctype="multipart/form-data" >
                                @csrf
                                <input type="hidden" name="invoice_id" value="{{$tax_invoice->id}}">
                                    ...
                                    <div class="card-body mt-2">
                                        <div class="row align-items-center">
                                            <div class="col-sm-3 form-group d-none">
                                                ...
                                                <select name="branch" class="common-select2" style="width: 100% !important" id="branch">
                                                    @foreach ($projects as $item)
                                                        <option value="{{ $item->id }}" selected>{{ $item->proj_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-sm-3 form-group" id="printarea">
                                                ...
                                                <input type="text" value="{{ convert_date_format($tax_invoice->date) }}" class="form-control date" name="date" id="date" required>
                                            </div>
                                            <div class="col-sm-3 form-group">
                                                ...
                                                <select name="pay_mode" id="pay_mode" class="common-select2" style="width: 100% !important" required>
                                                    ...
                                                    @foreach ($modes as $item)
                                                        <option value="{{ $item->title }}" {{$tax_invoice->pay_mode == $item->title?'selected':''}}>{{ $item->title }} </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-sm-4 form-group customer-select">
                                                ...
                                                <select name="customer_name" id="customer_name" class="common-select2 party-info customer" style="width: 100% !important" data-target="" required>
                                                    ...
                                                    @foreach ($customers as $customer)
                                                        <option value="{{ $customer->pi_code }}" {{$tax_invoice->customer_name==$customer->pi_code?'selected':''}}>{{ $customer->pi_name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-sm-2">
                                            ...
                                            <div class="col-sm-3 form-group">
                                                <label for="">TRN</label>
                                                <input type="text" class="form-control" name="trn_no" id="trn_no" class="form-control" readonly {{$tax_invoice->trn_no}}>
                                            </div>
                                            <div class="col-sm-3 form-group">
                                                <label for="">Contact Number</label>
                                                <input type="text" class="form-control" name="contact_no" id="contact_no" readonly value="{{$tax_invoice->contact_no}}">
                                            </div>        
                                            <div class="col-sm-3 form-group">
                                                <label for="">Address</label>
                                                <input type="text" class="form-control" name="address" id="address" readonly value="{{$tax_invoice->address}}">
                                            </div>        
                                            <div class="col-sm-3 form-group">
                                                <label for="">Invoice No</label>
                                                <input type="text" class="form-control" name="invoice_no" id="invoice_no" required readonly value="{{$tax_invoice->invoice_no}}">
                                            </div>
                                        ...
                                        <table class="table table-bordered table-sm ">
                                            <thead class="user-table-body">
                                                <tr>
                                                    ...
                                                    <th style="min-width: 120px !important;">Item Code</th>
                                                    <th>QTY</th>
                                                    <th>Rate</th>
                                                    <th>Amount</th>
                                                    <th>Vat</th>
                                            ...
                                            <tbody id="TBody" class="user-table-body">
                                                @foreach ($invoice_items as $invoice_i)
                                                    <tr class="purchase_tr">
                                                        <td>
                                                            <select name="multi_category[]" class="inputFieldHeight2 form-control multi_category" style="width: 100%; HEIGHT: 36PX;">
                                                                ...
                                                                @foreach ($categories as $item)
                                                                    <option style="width: 100px" value="{{ $item->id }}" {{$item->id==$invoice_i->cat_id?'selected':''}}>{{ $item->name }}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td class="multi_brand_td">
                                                            <select name="multi_brand[]" class="inputFieldHeight2 form-control multi_brand" style="width: 100%; HEIGHT: 36PX;">
                                                                <option value="">Select....</option>
                                                                @foreach ($invoice_i->item_brands($invoice_i) as $brand)
                                                                    <option value="{{$brand->id}}" {{$brand->id==$invoice_i->brand_id?'selected':''}}>{{$brand->name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td class="multi_vehicle_name_td">
                                                            <select name="multi_vehicle_name[]" class="inputFieldHeight2 form-control multi_vehicle_name" style="width: 100%; HEIGHT: 36PX;">
                                                                <option value="">Select....</option>
                                                                @foreach ($invoice_i->item_vehicle_names($invoice_i) as $vehicle_name)
                                                                    <option value="{{$vehicle_name->id}}" {{$vehicle_name->id==$invoice_i->vehicle_name_id?'selected':''}}>{{$vehicle_name->name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td class="multi_vehicle_model_td">
                                                            <select name="multi_vehicle_model[]" class="inputFieldHeight2 form-control multi_vehicle_model" style="width: 100%; HEIGHT: 36PX;">
                                                                <option value="">Select....</option>
                                                                @foreach ($invoice_i->item_model_names($invoice_i) as $sub_brand)
                                                                    <option value="{{$sub_brand->id}}" {{$sub_brand->id==$invoice_i->sub_brand_id?'selected':''}}>{{$sub_brand->name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td class="multi_item_code_td">
                                                            <select name="multi_item_code[]" class="inputFieldHeight2 form-control multi_item_code" style="width: 100%; HEIGHT: 36PX;">
                                                                <option value="">Select....</option>
                                                                @foreach ($invoice_i->item_codes($invoice_i) as $code)
                                                                    <option value="{{$code->id}}" {{$code->id==$invoice_i->item_code_id?'selected':''}}>{{$code->name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td><input type="number" name="multi_qty[]" step="any" class="inputFieldHeight2 form-control inputFieldHeight multi_qty" style="width: 100%;height:36px;" value="{{$invoice_i->quantity}}" required></td>
                                                        <td><input type="number" name="multi_unit_price[]" step="any" class="inputFieldHeight2 form-control inputFieldHeight multi_unit_price" style="width: 100%;height:36px;" value="{{$invoice_i->unit_price}}" required></td>
                                                        <td><input type="number" name="multi_total_price[]" step="any" class="inputFieldHeight2 form-control inputFieldHeight multi_total_price" style="width: 100%;height:36px;" readonly value="{{$invoice_i->price}}"></td>
                                                        <td><input type="number" name="multi_vat_price[]" step="any" class="inputFieldHeight2 form-control inputFieldHeight multi_vat_price" style="width: 100%;height:36px;" readonly value="{{$invoice_i->vat}}"></td>
                                                        <td><input type="number" name="multi_total_amount[]" step="any" class="form-control inputFieldHeight2 multi_total_amount"style="width: 100%;height:36px;" readonly value="{{$invoice_i->total_price}}"></td>
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td colspan="2">
                                                        <input type="text" readonly id="sub_total" class="inputFieldHeight2 form-control inputFieldHeight sub_total" name="amount" value="{{$tax_invoice->price}}" placeholder="Sub Total" readonly>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td colspan="7"></td>
                                                    ...
                                                    <td colspan="2">
                                                        <input type="text" readonly id="tax_amount" class="inputFieldHeight2 form-control inputFieldHeight tax_amount" name="vat_amount" value="{{$tax_invoice->vat_amount}}" placeholder="Tax Amount" readonly>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td colspan="7"></td>
                                                    ...
                                                    <td colspan="2">
                                                        <input type="text" readonly id="total_amount" class="inputFieldHeight2 form-control inputFieldHeight total_amount" name="total_amount" placeholder="Total Amount" readonly required value="{{$tax_invoice->total_price}}">
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    ...
                                    <div class="card-body bg-white">
                                        <div class="row px-1">
                                            <div class="col-sm-12 text-right d-flex justify-content-end mt-2 mb-1">
                                                <button type="button" class="btn btn-info formButton mr-1" id="sale_form_preview">
                                                    <div class="d-flex">
                                                ...
                                                <button type="button" class="btn btn-warning d-none" id="new_form_button"> <a href="{{route('taxInvoIssue')}}">New Entry</a> </button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script>
    ...
    function sum_all_amount(){
        ...
        $('.multi_total_price').each(function() {
            ...
            sum = sum+this_amount;
            // tax calculate
            vat_amount += (this_amount*Number(5))/100;
        });
    ...
    $(document).on('keyup', '.multi_unit_price', function(e){
        ...
        var vat_amount = (total_amount*Number(5))/100;
    ...
    $(document).on('click', '#sale_form_preview', function(e){
        e.preventDefault();
    ...
    $(document).on("change", ".multi_item_code", function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                multi_unit_price_obj.val(response.product.sale_price);
                if (response.stock) {
                    multi_qty_obj.attr('max', response.stock.pcs);
                    toastr.info("Avaialble Qty "+ response.stock.pcs,"Info");
                } else {
                    toastr.warning("Qty Not Avaialble ","Warning");
                    multi_qty_obj.attr('max', 0);
                    m_select.closest('.purchase_tr').find('.multi_item_code_td .common-select2').val('').trigger('change');
                }
            }
        })
    });
    $(document).on('keyup', '.multi_qty', function(e){
        ...
        if(Number(max)>0){
            if(Number(qty)>Number(max)){
                toastr.warning('Max sale qty '+max,"Warning");
                $(this).val(max);
            }
        }else{
            toastr.warning("Qty Not Avaialble ","Warning");
        ...
        if(!party){
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if(button_type == 'save'){
                        toastr.success('Sales successfully');                        
                    }else{
