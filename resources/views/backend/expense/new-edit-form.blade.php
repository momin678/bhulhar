            ...
            <div class="content-body">
                <div class="tab-content bg-white  p-2">
                    <section id="widgets-Statistics">
                        <h4>Expense</h4>
                            <form action="{{ route('expense.update', $edit_expense->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method("PUT")
                                <div class="match-height">
                                    <div class="row">
                                        <div class="col-md-4">
                                            ...
                                            <select name="master_acount" class="inputFieldHeight form-control common-select2" style="width: 100% !important" id="master_acount" required>
                                                ...
                                                @foreach ($master_accounts as $item)
                                                    <option value="{{ $item->mst_ac_code }}" {{ $edit_expense->master_acount_id == $item->mst_ac_code ? 'selected' : '' }}>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select name="account_head" class="inputFieldHeight form-control common-select2" style="width: 100% !important" id="account_head"
                                                ...
                                                @foreach ($account_heads as $item)
                                                    <option value="{{ $item->id }}" {{$edit_expense->account_head_id == $item->id ? 'selected': ''}}>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select name="party_name" class="inputFieldHeight form-control common-select2" style="width: 100% !important" required>
                                                ...
                                                @foreach ($parties as $item)
                                                    <option value="{{ $item->id }}" {{ $edit_expense->party_info_id == $item->id ? 'selected' : '' }}>
                                                        {{ $item->pi_name }}</option>
                                                @endforeach
                                            </select>
                                            @error('party_name')
                                        ...
                                        <div class="col-md-4">
                                            <label>Date</label>
                                            <input type="date" id="date" class="inputFieldHeight form-control" name="date" placeholder="date" required value="{{$edit_expense->date}}">
                                            @error('date')
                                                <div class="btn btn-sm btn-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-md-4">
                                            ...
                                            <input type="file" class="inputFieldHeight form-control" name="voucher_copy" value="{{$edit_expense->voucher_copy}}">
                                            @error('bank_name')
                                                <div class="btn btn-sm btn-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-md-4">
                                            ...
                                            <input type="text" id="signatory" class="inputFieldHeight form-control" name="taxable_amount" value="{{$edit_expense->taxable_amount}}" placeholder="Taxable Account" required>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input type="number" id="vat_amount" class="inputFieldHeight form-control"  name="vat_amount" value="{{ $edit_expense->vat_amount }}" placeholder="Vat Amount" required>
                                            @error('vat_amount')
                                                <div class="btn btn-sm btn-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-md-4">
                                            ...
                                            <input type="text" class="inputFieldHeight form-control" name="total_amount" placeholder="Total Amount" value="{{$edit_expense->total_amount}}" >
                                            @error('total_amount')
                                                <div class="btn btn-sm btn-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="mt-2 col-md-4">
                                            <div class="d-flex justify-content-end">
                                                <a href="{{route("new-expense")}}" class="btn mr-1 mAddNewIcon formButton" title="Add New">
                                                    <div class="d-flex">
                                                        <div class="formSaveIcon">
                                                            <img src="{{asset('assets/backend/app-assets/icon/add-icon.png')}}" alt="" srcset="" class="img-fluid" width="25">
                                                        </div>
                    ...
                    <section id="widgets-Statistics">
                        <div class="mt-2">
                            <div class="row mb-1">
                                <div class="col-md-6">
                                    <form>
                                    <input type="text" name="q" class="form-control input-xs pull-right ajax-search" placeholder="Search By Code, Name, Account Number" data-url="{{ route('admin.masterAccSearchAjax',$id="bankDetails") }}">
                                    </form>
                                </div>
                                <div class="col-md-6 text-right">
                                    <a href="{{ route('pdf',$id="bankDetails") }}" class="btn btn-xs mPrint formButton" target="_blank" title="Print"><img  src="{{asset('assets/backend/app-assets/icon/print-icon.png')}}" alt="" srcset="" class="img-fluid" width="30"> Print</a>
