                                ...
                                <div class="card-body pb-1">
                                    <div class="row mx-1 mt-1 d-flex justify-content-center">
                                        ...
                                        {{-- <div class="col-md-3 changeColStyle ">
                                            ...
                                            <div class="row align-items-center">
                                                 ...
                                                 <div class="col-9">
                                                     ...
                                                     class="common-select2" style="width: 100% !important" data-target="" required>
                                                         ...
                                                         <option value="{{ $invoice->id }}" data-from="sale-revenue">{{ $invoice->invoice_no }}</option>
                                                         @endforeach
                                         ...
                                         <input type="hidden" id="invoice-from" name="invoice_from">
                                        ...
                                        <div class="col-md-3 changeColStyle" id="printarea">
                                            <div class="row align-items-center">
                                                ...
                                                <div class="col-8">
                                                    <input
                                                    class="form-control inputFieldHeight  @error('voucher_file') is-invalid @enderror" type="file" name="voucher_file"
    ...
    $(document).ready(function() {
        ...
        $('#party_info').change(function() {
            if ($(this).val() != '') {
                ...
                var _token = $('input[name="_token"]').val();
                $.ajax({
                    url: "{{ route('partyInfodueInvoices') }}",
                    method: "POST",
                    data: {
                        value: value,
                        _token: _token,
                    ...
                    success: function(response) {
                        ...
                        $("#invoice_no").empty().append(response.page);
                    }
                })
            }
        });
        ...
        $('#invoice_no').change(function() {
            if ($(this).val() != '') {
                var invoice_from = $(this).find(':selected').data('from');
                $('#invoice-from').val(invoice_from);
                var value = $(this).val();
                var _token = $('input[name="_token"]').val();
                $.ajax({
                    url: "{{ route('findInvoiceforReceipt') }}",
                    method: "POST",
                    data: {
                    ...
                    success: function(response) {
                        ...
                        $("div.search-item-pi select").val(response.info.id);
                        $("#trn_no").val(response.info.trn_no);
                        $("#pi_code").val(response.info.pi_code);
                        $("#due_amount").val(due);
                        $("#pay_amount").attr({
                                        "max" : due,        // substitute your own
                                        "min" :  0.1         // values (or variables) here
                                    });
