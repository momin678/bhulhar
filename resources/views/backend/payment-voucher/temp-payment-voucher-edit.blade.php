    ...
    <div class="app-content content print-hideen">
        ...
        <div class="content-wrapper">
            <div class="content-body">
                @include('clientReport.accounting._header', ['activeMenu' => 'payment-report'])
                                ...
                                <div class="cardStyleChange bg-white">
                                    <div class="card-body pb-1">
                                        <div class="row px-1 mt-1">
                                            <div class="col-md-6 changeColStyle search-item-pi">
                                            ...
                                            <div class="card-body bg-white" id="table-part">
                                                ...
                                                <table class="table table-bordered table-sm " >
                                                    ...
                                                    @foreach ($new_expenses as $key => $item)
                                                        @php
                                                            $exit_invoice_amount = App\TempPaymentVoucherDetail::where('come_from', 'expenses')->where('sale_id', $item->id)->where('payment_id', $temp_payment_voucher->id)->first();
                                                        ...
                                                        <tr id="TRow" class="tr_obj">
                                                            <td>
                                                                <input type="checkbox" class="checkbox-record id_input" name="records[]" value="{{$item->id}}" {{$exit_invoice_amount && $exit_invoice_amount->checked=='yes'?'checked':''}}>
                                                                <input type="checkbox" class="checkbox-record type_input" name="type_input[]" value="expenses" style="display: none !important;" {{$exit_invoice_amount && $exit_invoice_amount->checked=='yes'?'checked':''}}>
                                                            </td>
                                                            ...
                                                            <td>
                                                                <input type="number" step="any" name="due_pay_amount[]" style="width: 150px;" class="due_pay_amount" max="{{$due_amount}}" value="{{$exit_invoice_amount && $exit_invoice_amount->checked=='yes'?$exit_invoice_amount->total_amount:''}}">
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                    @foreach ($new_supplier as $key => $item)
                                                        @php
                                                            $exit_invoice_amount = App\TempPaymentVoucherDetail::where('come_from', 'service_expense')->where('sale_id', $item->id)->where('payment_id', $temp_payment_voucher->id)->first();
                                                            $due_amount = $item->due_amount;
                                                            // dd($exit_invoice_amount);
                                                            if($exit_invoice_amount){
                                                                $due_amount += $exit_invoice_amount->total_amount;
                                                        ...
                                                        @endphp
                                                        {{-- @dump($exit_invoice_amount) --}}
                                                        <tr id="TRow" class="tr_obj">
                                                            <td>
                                                                <input type="checkbox" class="checkbox-record id_input" name="records[]" value="{{$item->id}}" {{$exit_invoice_amount && $exit_invoice_amount->checked=='yes'?'checked':''}}>
                                                                <input type="checkbox" class="checkbox-record type_input" name="type_input[]" value="service_expense" style="display: none !important;" {{$exit_invoice_amount && $exit_invoice_amount->checked=='yes'?'checked':''}}>
                                                            </td>
...
@push('js')
    ...
    <script>
        ...
        $(document).on('click', '.id_input', function(){
            tr_obj = $(this).closest('.tr_obj')
            if(this.checked == true){
                tr_obj.find('.type_input').prop('checked', true);
            }else{
                tr_obj.find('.type_input').prop('checked', false);
                tr_obj.find('.due_pay_amount').val('');
            }
            total_pay = 0;
            $('.due_pay_amount').each(function(){
                var this_amount = $(this).val();
                this_amount = (this_amount === '') ? 0 : this_amount;
                this_amount = Number(this_amount);
                total_pay = total_pay + this_amount;
            })
            if(total_pay>0){
                $('#pay_amount').val(total_pay.toFixed(2));
            }
        })
        $(document).on('keyup', '.due_pay_amount', function(){
            tr_obj = $(this).closest('.tr_obj');
            if(Number($(this).val())>0){
                tr_obj.find('.type_input').prop('checked', true);
                tr_obj.find('.id_input').prop('checked', true);
            }else{
                tr_obj.find('.type_input').prop('checked', false);
                tr_obj.find('.id_input').prop('checked', false);
            }
            total_pay = 0;
            $('.due_pay_amount').each(function(){
                var this_amount = $(this).val();
