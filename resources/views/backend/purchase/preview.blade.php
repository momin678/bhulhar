            ...
            <div class="">
                <div class="mx-2 mb-2">
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <strong>Invoice No:</strong> {{ $requests['supplier_invoice']}}
                        </div>
                        <div class="col-md-2">
                            <strong>Date:</strong> {{ date('d/m/Y', strtotime($requests['date']))}}
                        </div>
                        <div class="col-md-2">
                            <strong>Pay Mode:</strong> {{ $requests['pay_mode']}}
                        </div>
                        <div class="col-md-3">
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered border-botton">
                            ...
                            <tbody class="user-table-body">
                                @foreach ($multi_category as $key => $item)
                                    @if ($item)
                                        <tr class="border-bottom text-center">
                                            @php
                                                $category = App\Category::find($item);
                                                $brand = App\Brand::find($multi_brand[$key]);
                                                $vehicle_name = App\VehicleName::find($multi_vehicle_name[$key]);
                                                $sub_brand = App\SubBrand::find($multi_vehicle_model[$key]);
                                                $item_code = App\ItemCode::find($multi_item_code[$key]);
                                            @endphp
                                            <td>{{$category?$category->name:''}}</td>
                                            <td>{{$brand?$brand->name:''}}</td>
                                            <td>{{$vehicle_name?$vehicle_name->name:''}}</td>
                                            <td>{{$sub_brand?$sub_brand->name:''}}</td>
