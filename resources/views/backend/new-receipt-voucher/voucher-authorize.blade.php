    ...
    <div class="content-wrapper">
        <div class="content-body">
            <div class="nav nav-tabs master-tab-section" id="nav-tab" role="tablist">
                    ...
                    <a  href="{{route("receipt-voucher-authorize")}}" class="nav-item nav-link active" role="tab" aria-controls="nav-contact" aria-selected="false" id="mJournalAuthorizationSection">
                        <div class="master-icon text-cente">
            ...
            <div class="tab-content bg-white">
                <div id="journalAuthorization" class="tab-pane active">
                    <section id="widgets-Statistics">
                        <div class="col-md-12">
                            <div class="row">
                                <h4 class="mt-2 ml-2">Receipt Voucher to Authorised</h4>
                            </div>
                        </div>
                    </section>
                    <section>
                                ...
                                <tbody class="user-table-body">
                                    @foreach ($vouchers as $voucher)
                                        <tr class="trFontSize">
                                            ...
                                            {{-- <td>{{ $voucher->journal_no }}</td> --}}
                                            {{-- <td class="pl-2">{{ $voucher->voucher_type->type == 'DR' ? 'DEBIT' : ($voucher->voucher_type->type=='CR' ? 'CREDIT' : 'JOURNAL') }}</td> --}}
                                            <td>{{ $voucher->narration }}</td>
                                            <td>{{ $voucher->amount }}</td>
                                            <td style="padding-bottom: 11px; padding-top: 0px">
                                                <div class="d-flex justify-content-end">
                                                    <a href="#" class="btn journalAuthorizeShowId" style="height: 30px; width: 30px;" title="Preview" id="{{$voucher->id}}">
                                                        <img src="{{ asset('assets/backend/app-assets/icon/view-icon.png')}}" style=" height: 30px; width: 30px;">
                                                    </a>
                                                    @if ($voucher->created_by == Auth::id())
                                                        <a href="{{route('receipt-voucher-edit', $voucher->id)}}" class="btn" style="height: 30px; width: 30px;" title="Edit" id="{{$voucher->id}}">
                                                            <img src="{{ asset('assets/backend/app-assets/icon/edit-icon.png')}}" style=" height: 30px; width: 30px;">
                                                        </a>
                                                    @endif
                                                    <a href="{{route('receipt-voucher-rejected', $voucher->id)}}" class="btn" style="height: 30px; width: 30px;" title="Rejected">
...
<script>
    $(document).on("click", ".journalAuthorizeShowId", function(e) {
        ...
        console.log(id);
		$.ajax({
			url: "{{URL('receipt-voucher-authorize-modal')}}",
			type: "post",
			cache: false,
			data:{
				_token:'{{ csrf_token() }}',
