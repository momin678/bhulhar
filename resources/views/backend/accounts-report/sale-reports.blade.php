                                                ...
                                                {{-- @dump($invoice->tasks()) --}}
                                                    @foreach ($invoice->items as $key => $invoice_item)
                                                        <tr class="text-center trFontSize">
                                                            @if ($key == 0)
                                                                <td rowspan="{{count($invoice->items)}}">{{$invoice->invoice_no}}</td>
                                                            @endif

                                                            <td>{{$invoice_item->task_name}}</td>
                                                            @if ($key == 0)
                                                                <td rowspan="{{count($invoice->items)}}">{{date('d-m-Y',strtotime($invoice->date))}}</td>
                                                            @endif
                                                            <td>{{$invoice_item->budget}}</td>
                                                            <td>{{ $invoice_item->vat->value  }} {{ $invoice_item->vat->name }}</td>
