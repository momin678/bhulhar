...
<section class=" border-bottom" style="padding: 5px 15px;background:#364a60;">
    <div class="d-flex flex-row-reverse">
        ...
        <div class="pr-1 w-100 pl-2">
            <h4 style="font-family:Cambria;font-size: 2rem;color:white;">Bill List</h4>
        </div>
    </div>
</section>
        ...
        <table class="table table-bordered table-sm text-center">
            ...
            <tbody id="purch-body">
                @foreach ($expenses as $item)
                    <tr style="text-align:center;" class="tr-item">
                        <td class="purch_exp_view" id="{{ $item->id }}">{{ date('d/m/Y', strtotime($item->date)) }}</td>
                        <td class="purch_exp_view" id="{{ $item->id }}">{{ $item->purchase_no }}</td>
                        <td class="purch_exp_view" id="{{ $item->id }}">{{ $item->invoice_no }}</td>
                        <td class="purch_exp_view" id="{{ $item->id }}">{{ $item->party?$item->party->pi_name:'' }}</td>
                        <td class="purch_exp_view" id="{{ $item->id }}">{{ $item->total_amount }}</td>
                        <td style="padding-bottom: 11px; padding-top: 0px">
                            <div class="d-flex justify-content-center">
                                <a href="#"  data="{{ 'purchase-approve2/'. $item->id }}" class="btn btn-icon btn-warning pandding-accounting-approve" onclick="return confirm('Approve! Confirm?')" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Approve"><i class="bx bx-check"></i></a>
                            </div>
                        </td>
                    </tr>
                @endforeach
