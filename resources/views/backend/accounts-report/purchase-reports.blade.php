                        ...
                        <div class="content-body">
                            <section id="widgets-Statistics">
                                <div class="d-flex justify-content-between align-items-center print-hideen">
                                    @include('clientReport.report._financial_report_subheader', [ 'activeMenu' => 'purchase_reports'])
                                </div>
                                        ...
                                        <table class="table mb-0 table-sm table-bordered">
                                            <thead  class="thead">
                                                <tr class="text-center" style="height: 40px;">
                                                    ...
                                                    <th>Amount</th>
                                                    <th>Vat</th>
                                                    <th>Total Amount <small>(@if(!empty($currency->symbole)){{$currency->symbole}}@endif)</small></th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @php
                                                    ...
                                                    $qty = 0;
                                                @endphp
                                                @foreach ($purchases as $invoice_key => $invoice)
                                                {{-- @dump($invoice->purchase_no) --}}
                                                {{-- @dump($invoice->tasks()) --}}
                                                    @foreach ($invoice->items($invoice->id) as $key => $invoice_item)
                                                        <tr class="text-center trFontSize">
                                                            @if ($key == 0)
                                                                <td rowspan="{{count($invoice->items($invoice->id))}}">{{date('d-m-Y',strtotime($invoice->date))}}</td>
                                                            @endif
