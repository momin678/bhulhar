...
<section class=" border-bottom" style="padding: 5px 15px;background:#364a60;">
    <div class="d-flex flex-row-reverse">
        ...
        <div class="pr-1 w-100 pl-2">
            <h4 style="font-family:Cambria;font-size: 2rem;color:white;">Fund Collection</h4>
        </div>
    </div>
</section>
        ...
        <table class="table table-bordered table-responsive-sm table-sm text-center">
            <thead class="thead">
                <tr >
                    ...
                    <th style="width: 150px;">Amount <small>(@if(!empty($currency->symbole)){{$currency->symbole}}@endif)</small></th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody id="purch-body">
                @foreach ($fund_collections as $item)
                <tr class="tr-item">
                    <td class="text-center fund-show"  id="{{$item->id}}">{{date('d/m/Y',strtotime($item->date))}}</td>
                    <td class="text-center fund-show"  id="{{$item->id}}">{{$item->party?$item->party->pi_name:''}}</td>
                    <td class="text-center fund-show"  id="{{$item->id}}">{{$item->account_head?$item->account_head->fld_ac_head:''}}</td>
                    <td class="text-center fund-show"  id="{{$item->id}}">{{$item->fund_number}}</td>
                    <td class="text-center fund-show"  id="{{$item->id}}">{{number_format($item->amount,2)}}</td>
                    <td style="padding-bottom: 11px; padding-top: 0px">
                        <div class="d-flex justify-content-center">
                            <a href="#" data="{{ 'fund-collection-approval2/'. $item->id }}" class="btn btn-icon btn-warning pandding-accounting-approve" onclick="return confirm('Approve! Confirm?')" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Approve"><i class="bx bx-check"></i></a>
                        </div>
                    </td>
                </tr>
                @endforeach
