...
<section class=" border-bottom" style="padding: 5px 15px;background:#364a60;">
    <div class="d-flex flex-row-reverse">
        ...
        <div class="pr-1 w-100 pl-2">
            <h4 style="font-family:Cambria;font-size: 2rem;color:white;">Payment List</h4>
        </div>
    </div>
</section>
    ...
    <div class="pt-2">
        <table class="table table-bordered table-sm mt-2 text-center">
            <thead class="thead">
                <tr >
                    <!-- <th  style="width: 6%">#</th> -->
                    <th  style="width: 22%">Payment No</th>
            ...
            <tbody>
                ...
                <tr class="tr-item">
                    <td class="purch_exp_view"  id="{{$item->id}}">{{$item->payment_no}}</td>
                    <td class="purch_exp_view"  id="{{$item->id}}">{{$item->party?$item->party->pi_name:''}}</td>
                    <td class="purch_exp_view"  id="{{$item->id}}">{{$item->narration}}</td>
                    <td class="purch_exp_view"  id="{{$item->id}}">{{number_format($item->total_amount,2)}}</td>
                    <td style="padding-bottom: 11px; padding-top: 0px">
                        <div class="d-flex justify-content-center">
                            <a href="#" data="{{'payment-voucher-approve2/'. $item->id }}" class="btn btn-icon btn-warning pandding-accounting-approve" onclick="return confirm('Approve! Confirm?')" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Approve"><i class="bx bx-check"></i></a>
                        </div>
                    </td>
                </tr>
