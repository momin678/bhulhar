...
@php
    ...
    $per_day_salary = number_format($basic_salary->sum('amount')/$this_month_days,2);
    $per_hours_salary = ($basic_salary->sum('amount')/$this_month_days)/10;
    $less_absent_amount = count($absent_count)*$per_day_salary;
    $less_advance_amount = App\AdvanceSalary::where('employee_id', $employee->id)->sum('due');
    $net_payable_amount = $total_salary->sum('amount') - ($less_absent_amount+$less_advance_amount);
                    ...
                    <div class="daily-attendance-report">
                        ...
                        <table class="table table-sm table-bordered">
                            ...
                            <tr>
                                ...
                                <td colspan="4">{{number_format($less_absent_amount,2,'.','')}}</td>
                            </tr>
                            <tr>
                                <td colspan="9">Less Advance Paid</td>
                                <td colspan="4">{{number_format($less_advance_amount,2,'.','')}}</td>
                            </tr>
                            <tr>
                                <td colspan="9">Balance Amount Payable</td>
                                <td colspan="4">{{number_format($net_payable_amount,2,'.','')}}</td>
                            </tr>
                        </table>
                    </div>
                </div>
