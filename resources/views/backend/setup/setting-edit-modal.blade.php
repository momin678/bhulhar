...
<section id="basic-vertical-layouts">
    <div class="cardStyleChange">
        <div class="content-body">
            <form class="form form-vertical" action="{{ isset($edit_setup) ? route('company-setup.update', $edit_setup->id) : route('company-setup.store')}}" method="POST" enctype="multipart/form-data">
                @csrf 
                @isset($edit_setup)
                    @method('PUT')
                @endisset
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 col-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" id="config-name" class="inputFieldHeight form-control @error('name') error @enderror" name="name" value="{{ isset($edit_setup) ? $edit_setup->name : old('name')}}" placeholder="Setup Name" required>
                                                @error('name')
                                        ...
                                        <div class="col-md-12 col-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" id="config-value-text"  class="form-control @error('value') error @enderror" name="value" value="{{ isset($edit_setup) ? $edit_setup->value : old('value')}}" placeholder="Setup Value" style="display: {{ (isset($edit_setup) && $edit_setup->config_type =='img') ? 'none': 'block' }}">
                                                @error('value')
                                                <span class="error">{{ $message }}</span>
                                                @enderror
                                            </div>
