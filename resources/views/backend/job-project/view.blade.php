        ...
        <div class="col-md-12">
            <div class="customer-info">
                <div class="row ml-1 mr-1" style="border:1px solid #bdbdbd">
                    ...
                    <div class="col-md-6 customer-dynamic-content">
                        ...
                        {{ $project->attention ? $project->attention : '.'}}<br>
                        @if ($project->party->phone_no==$project->party->con_no && $project->party->con_no!='.' &&  $project->party->con_no!='')
                        {{$project->party->phone_no}}
                        @elseif($project->party->con_no && $project->party->phone_no && $project->party->con_no!='.' && $project->party->phone_no!='.')
                        {{$project->party->con_no.', '. $project->party->phone_no}}
                        @else
                        {{$project->party->con_no && $project->party->con_no!='.'? $project->party->con_no:($project->party->phone_no?$project->party->phone_no:'.')}}
                    ...
                    <div class="col-md-4 customer-dynamic-content text-right pt-1">
                        <span>
                            L.P.O NO: {{$project->lpo_no}}<br>
                        </span>
                        <span>
                            D.O NO: {{$project->do_no}}<br>
                        </span>
                        <span>
                            Date: {{date('d/m/Y')}} <br>
                        </span>
            ...
            <div class="my-1 px-1 print-table table-responsive">
                <table class="table table-sm">
                    <thead style="background: #ddd !important">
                        <tr>
                            ...
                            <th class="text-center" style="color:#444;font-weight:600;"> Rate </th>
                            <th class="text-center" style="color:#444;font-weight:600;"> Discount  </th>
                            <th class="text-center" style="color:#444;font-weight:600;"> Amount </th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            <td colspan="6" class="text-dark text-right"
                        ...
                        <tr>
                            ...
                            <td class="text-center text-dark"
                                style="border-right:1px solid #f2dede;font-size:14px;font-weight:500;">
                                {{ number_format($project->total_budget-$project->advance_amount,2,'.','') }}
                            </td>
                        </tr>
                ...
                <div class="container mt-1 ">
                    ...
                    @if(in_array($project->extension, $pdfExtensions))
                    ...
                    <div class="row justify-content-center">
                    <a href="{{ Storage::url('upload/documents/' . $project->voucher_file) }}" target="_blank">View
                        <img src="{{asset('icon/pdf-download-icon-2.png')}}" alt="Image" class="img-fluid" style="height: 50px"></a>
                    {{-- <iframe src="{{ Storage::url('upload/documents/' . $project->voucher_file) }}" width="100%" height="600px" frameborder="0"></iframe> --}}
                    </div>
                    @else
                    <div class="row justify-content-center">
                        <div class="col-auto">
                           <a href="{{ asset('storage/upload/documents/' . $project->voucher_file) }}" target="_blank"> <img src="{{ asset('storage/upload/documents/' . $project->voucher_file) }}" alt="Image" class="img-fluid" ></a>
                        </div>
                    </div>
                    @endif
                    @endif
...
<p style="font-size:16px; line-hight:0 !important;font-weight:500;color:#000;margin:5px 20px 0 0;"> Utpal Barua:
056 583 1841 </p>
