        ...
        <div class="card-body content-padding">
            <div class="row">
                ...
                <div class="col-sm-3 col-12">
                    ...
                    <input type="date" class="form-control"  value="{{date('d-m-Y', strtotime($purchase_info->date))}}">
                </div>
                ...
                <div class="card-body content-padding">
                    <div class="row">
                        ...
                        <div class="col-sm-3 col-12">
                            ...
                            <input type="hidden" class="form-control vat_rate" max="" name="vat_rate" id="vat_rate">
                            <input type="hidden" class="form-control" max="" name="received_qty" id="qty1">

                            <input type="number" class="form-control quantity_vlaue quantity" max="" name="quantity" id="qty">
                            <span class="text-danger" id="quantityErrorMsg"></span>
                        </div>
                        <div class="col-sm-3 col-12">
                            <label for="purchase_rate">Purchase Rate</label>
                            <input type="text" class="form-control purchase_rate" name="purchase_rate" id="purchase_rate" readonly step=".01" oninput="validate(this)">
...
<table class="table table-sm table-bordered">
    <thead>
        <tr>
            ...
            <th scope="col">Qty</th>
            <th scope="col">Remainining quantity</th>
    ...
    <tbody id="tempLists" class="user-table-body">
        ...
        @php
            ...
            $QTY = PurchaseReturnDetail::where("purchase_no", $item->purchase_no)->where("item_id",$item->item_id)->get();
            $total_qty = $item->quantity - $QTY->sum('return_qty');
        @endphp
        <tr class="data-row">
            ...
            <td> <a type="button" class="btn btn-sm btn-warning image-edite"data-rqty="{{$item->quantity}}" data-style_name="{{$item->itemName->style_name}}" data-item_name="{{$item->itemName->item_name}}" data-quantity="{{$total_qty}}" data-vat_rate="{{$item->vat_rate}}" data-unit_price="{{$item->purchase_rate}}" data-vat_amount="{{number_format((float)( $item->vatRate->name),'2','.','')}}" data-cost_price="{{number_format((float)( $item->total_amount),'2','.','') }}" data-item_id="{{$item->item_id }}">Return</a></td>
        </tr>
        @endforeach
    ...
    <script>
           ...
           $(document).on('keyup', '.quantity_vlaue', function(e) {

            e.preventDefault();
           // alert('total_amount')

            let quantity1 =  $(".quantity").val();
            let pamount1 = $(".purchase_rate").val();
            let vatrate1 = $(".vat_rate").val();
            if (vatrate1 == 1) {
                vat_rate1 = 5;
            } else {
                vat_rate1 =  0;
            }
            let amount1 = quantity1 * pamount1;
            let total_vat_amount1 = (vat_rate1 * amount1) / 100;
            let total_amount = total_vat_amount1+amount1;
