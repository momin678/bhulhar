...
class JournalEntryController extends Controller
{
    ...
    public function journalEntryPost(Request $request)
    {
        ...
        $multi_head=$request->multi_acc_head;
        // dd($multi_head);
        ...
        $journal->transection_type  = $request->transection_type;
        ...
        $journal->total_amount      = $request->total_amount-$request->gst_total;
        ...
        $type='';
        // dd($multi_head);
        foreach($multi_head as $key => $each_head){
            $ac_head= AccountHead::find($each_head);
            if($ac_head){
                ...
                $jl_record->invoice_no          = $request->invoice_no[$key];
                $jl_record->account_head_id     = $request->multi_acc_head[$key];
                $jl_record->master_account_id   = $ac_head->master_account_id;
                $jl_record->account_head        = $ac_head->fld_ac_head;
                $jl_record->amount              = $request->multi_total_amount[$key]-$request->multi_tax_rate[$key];
                $jl_record->total_amount        =  $request->multi_total_amount[$key];
                $jl_record->gst_subtotal        =  $request->multi_tax_rate[$key];
                $jl_record->gst_amount        =  $request->multi_amount[$key];
                ...
                if($request->transection_type=='Purchase'){
                    $mst_head = MasterAccount::find(3);
                }elseif($request->transection_type=='Expense'){
                    $mst_head = MasterAccount::find(4);
                }elseif($request->transection_type=='Sales Entry'){
                    $mst_head = MasterAccount::find(5);
                }elseif($request->transection_type=='Payment Voucher'){
                    $mst_head = MasterAccount::find(3);
                }elseif($request->transection_type=='Receipt Voucher'){
                    $mst_head = MasterAccount::find(1);
                }
                $accHeadL = AccountHead::where('ma_code', $mst_head->mst_ac_code)->latest()->first();
                ...
                $accHead->ma_code = $mst_head->mst_ac_code;
                $accHead->fld_ac_head = $each_head;
                $accHead->fld_ac_code = $mst_head->mst_ac_code . "-" . $accHead->ac_code;
                $accHead->fld_ms_ac_head = $mst_head->mst_ac_head;
                $accHead->fld_definition = $mst_head->mst_definition;
                $accHead->account_type_id= $mst_head->account_type_id;
                $accHead->master_account_id= $mst_head->id;
                $accHead->save();
                $ac_head = $accHead;
        ...
        if($request->pay_mode=='Cash' || $request->pay_mode=='Card'){
            ...
            if($request->transection_type=='Purchase/Expense Entry'){
    ...
    public function journalEntryEditPost(Request $request,$journal)
    {
        ...
        foreach($multi_head as $key => $each_head){
            ...
            if($ac_head){
                ...
                $jl_record->amount              = $request->multi_total_amount[$key]-$request->multi_tax_rate[$key];
                $jl_record->total_amount        =  $request->multi_total_amount[$key];
                $jl_record->gst_subtotal        =  $request->multi_amount[$key];
                $jl_record->gst_amount        =  $request->multi_tax_rate[$key];
    ...
    public function journalMakeApprove($journal)
    {
        ...
        $ApproveJournal->voucher_scan2=$journal->voucher_scan2;
        ...
        if($ApproveJournal->transection_type=='Purchase' || $ApproveJournal->transection_type=='Expense')
        {
            $sub_invoice = Carbon::now()->format('Ymd');
            // return $sub_invoice;
            $latest_journal_no = JournalTemp::withTrashed()->whereDate('created_at', Carbon::today())->where('journal_no', 'LIKE', "%{$sub_invoice}%")->latest('id')->first();
            ...
            if ($latest_journal_no) {
                ...
                // return $sub_invoice;
                $let_purch_exp = PurchaseExpense::whereDate('created_at', Carbon::today())->where('purchase_no', 'LIKE', "%{$sub_invoice}%")->latest('id')->first();
                if ($let_purch_exp) {
                    $purch_no = $let_purch_exp->purchase_no + 1;
                } else {
                    $purch_no = Carbon::now()->format('Ymd') . '001';
                }

            $purch_ex=new PurchaseExpense();
            $purch_ex->date= $ApproveJournal->date;
            $purch_ex->pay_mode=  $ApproveJournal->pay_mode;
            $purch_ex->purchase_no= $purch_no;
            $purch_ex->invoice_no= 0;
            $purch_ex->project_id= $ApproveJournal->project_id;
            $purch_ex->head_id= 0;
            $purch_ex->Total_amount= $ApproveJournal->total_amount;
            $purch_ex->vat= $ApproveJournal->vat_amount;
            $purch_ex->amount= $ApproveJournal->amount;
            $purch_ex->party_id=  $ApproveJournal->party_info_id;
            $purch_ex->narration= $ApproveJournal->narration;
            $purch_ex->gst_subtotal=$ApproveJournal->without_gst;
            $purch_ex->paid_amount= $ApproveJournal->pay_mode=='Credit'?  0:$purch_ex->amount;
            $purch_ex->due_amount= $ApproveJournal->pay_mode=='Credit'?  $purch_ex->amount:0;
            $purch_ex->voucher_scan=$ApproveJournal->voucher_scan;
            $purch_ex->voucher_scan2=$ApproveJournal->voucher_scan2;
            $purch_ex->save();

            foreach($ApproveJournal->records->where('is_main_head',1) as $rec)
            {
                $purc_exp_itm=new PurchaseExpenseItem();
                $purc_exp_itm->head_id=$rec->account_head_id;
                $purc_exp_itm->invoice_no=$rec->invoice_no;
                $purc_exp_itm->vat=$rec->gst_subtotal;
                $purc_exp_itm->amount= $rec->amount;
                ...
                $purc_exp_itm->gst_subtotal= $rec->gst_amount;
                $purc_exp_itm->save();
            }

        }elseif($ApproveJournal->transection_type=='Sales Entry')
        {

        }elseif($ApproveJournal->transection_type=='Payment Voucher')
        {
            ...
            $payment=new Payment();
            $payment->date= $ApproveJournal->date;
            $payment->pay_mode=  $ApproveJournal->pay_mode;
            $payment->payment_no= $payment_no;
            $payment->head_id= 0;
            $payment->Total_amount=$ApproveJournal->amount;
            $payment->vat= 0;
            $payment->party_id=  $ApproveJournal->party_info_id;
            $payment->narration= $ApproveJournal->narration;
            $payment->paid_amount= 0;
            $payment->due_amount= 0;
            $payment->voucher_scan=$ApproveJournal->voucher_scan;
            $payment->voucher_scan2=$ApproveJournal->voucher_scan2;
            $payment->save();

            $cus=PartyInfo::find($ApproveJournal->party_info_id);
            $cus->balance=$cus->balance-$ApproveJournal->amount;
            $cus->save();

        }elseif($ApproveJournal->transection_type=='Receipt Voucher')
        {
    ...
    public function new_journal(Request $request){
        ...
        if($request->date){
            ...
            $journals = $journals->where('date',  $date);
            if($request->mVoucherType){
        ...
        if($request->from && $request->voucherType){
            ...
            $journals = $journals->whereBetween('date', [$from, $request->to]);
            $journals = $journals->whereHas('voucher_type', function(Builder $query) use ($voucherType){
                $query->where('type', $voucherType);
            });
        }
        ...
        if($request->date){
            ...
            $journals_temp = $journals_temp->where('date', $date);
            if($request->mVoucherType){
        ...
        //   need for download excel
        // $journals = Journal::with('records')->whereBetween('date', ['2024-04-01', '2024-06-30'])
        //             ->leftJoin('journal_records', 'journal_records.journal_id', '=', 'journals.id')
        //             ->WhereIn('journal_records.master_account_id',[8,9,10,11]) // account head id for income
        //             // ->WhereIn('journal_records.master_account_id',[13,14,15,16,17,18,22]) // account head id for expense
        //             ->orderBy('journals.date', 'asc')
        //             ->select('journals.*')->distinct()
        //             ->get();
        // return view('backend.journal.journal-excel-formate', compact('journals','projects', 'journal_no', 'modes', 'terms', 'cCenters', 'txnTypes', 'acHeads', 'vats', 'pInfos'));
    }
    ...
    public function journal_edit($id){
        ...
        $pInfos = PartyInfo::all();
        $vats = VatRate::orderBy('id','desc')->get();
        if($journalF->transection_type=='Purchase')
        {
            $acHeads = AccountHead::where('account_type_id','1')->where('fld_definition','=','Sell of Asset')->get();
        }
        elseif($journalF->transection_type=='Expense')
        {
            $acHeads = AccountHead::where('account_type_id','4')->get();
        }
    ...
    {
        if($request->value=='Purchase')
        {
            $acHeads = AccountHead::where('account_type_id','1')->where('fld_definition','=','Sell of Asset')->get();
        }
        elseif($request->value=='Expense')
        {
            $acHeads = AccountHead::where('account_type_id','4')->get();
        }
