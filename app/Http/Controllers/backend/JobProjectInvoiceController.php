...
class JobProjectInvoiceController extends Controller
{
    ...
    public function projectInvoiceCreate($id)
    {
        ...
        }elseif(($job_project->tasks->count() == $job_project->tasks->sum('is_invoice')) && $job_project->tasks->count()>0 && $job_project->type=='quotation'){
            return redirect()->route('project.invoice.index')->with(['alert-type' => 'warning', 'message' => "Invoice Already Exist"]);
        }else{
    ...
    public function store(Request $request)
    {
       ...
       // return($request);
        ...
        $tem_invoice_data = $request->only('job_project_id', 'customer_id','invoice_type','discount_amount','tax_invoice','attention');

        $tem_invoice_data['budget'] = $request->invoice_amount - $request->vat;
        $tem_invoice_data['invoice_no'] = $this->invoice_no();
    ...
    public function update(Request $request, JobProjectTemInvoice $tem_invoice)
    {
        ...
        return redirect('project/job/project/invoice')->with(['alert-type' => 'success','message' => 'The invoice has been updated Successfully']);
    }
    ...
    public function approvedInvoice(Request $request){
        ...
        if($request->filter_date){
            ...
            $invoices = JobProjectInvoice::where('invoice_from','project')->where('invoice_no','like','%'.$search . '%')->orderBy('id','DESC')->paginate(20);
        }else{
            $invoices = JobProjectInvoice::with('tasks')->where('invoice_from','project')->orderBy('invoice_no','DESC')->paginate(20);
        }
