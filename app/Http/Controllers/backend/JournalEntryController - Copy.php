...
class JournalEntryController extends Controller
{
    ...
    public function new_journal(Request $request){
        ...
        if($request->date){
            ...
            $journals = $journals->where('date', $new_date);
            if($request->mVoucherType){
        ...
        if($request->from && $request->to && $request->voucherType){
            ...
            $journals = $journals->whereBetween('date', [$new_date, $new_date2]);
            $journals = $journals->whereHas('voucher_type', function(Builder $query) use ($voucherType){
                $query->where('type', $voucherType);
            });
        }
        ...
        if($request->date){
            ...
            $journals_temp = $journals_temp->where('date', $new_date);
            if($request->mVoucherType){
        ...
        if($request->from && $request->from && $request->voucherType){
            $old_date = explode('/', $request->from);
            $new_data = $old_date[0].'-'.$old_date[1].'-'.$old_date[2];
            $new_date = date('Y-m-d', strtotime($new_data));
            ...
            $journals_temp = $journals_temp->whereBetween('date', [$new_date, $new_date2]);
            $journals_temp = $journals_temp->whereHas('voucher_type', function(Builder $query) use ($voucherType){
                $query->where('type', $voucherType);
            });
        }
