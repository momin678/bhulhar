...
class CustomerInvoiceController extends Controller
{
    ...
    public function customer_invoice(Request $request){
       ...
       elseif (Auth::user()->hasPermission('app.invoice.invoice_authorize'))
        {
            return redirect('customer-inv/invoice-athurization-list');
       }
       elseif (Auth::user()->hasPermission('app.invoice.invoice_approval'))
       {
           return redirect('customer-inv/invoice-approval-list');
      }
      else
      {
    ...
    //   dd($request->all());
      if(empty($request->item_ids)){
        ...
        $tax_invoice->new_invoice_no= $new_invocie_no;
        ...
        return redirect('customer-inv/customer-invoice')->with($notification);
    }
    ...
    public function invoice_print($id){
        ...
        $value = "values";
        $terms = PayTerm::get();
        return view('backend.customer-invoice.invoice', compact('invoice', 'value', 'terms'));
    }
    ...
    public function authorize_invoice($id)
    {
        ...
        if($pre_inv->status=="Authorize")
       {
        ...
        return redirect('customer-inv/invoice-athurization-list')->with($notification);
       }
       else
       {
        Gate::authorize('app.invoice.invoice_approval');
        ...
        $journal->amount            = $tax_invoice->amount+$tax_invoice->vat_amount+$tax_invoice->discount_amount+$tax_invoice->total_toll_fee;
        $journal->tax_rate          = $v_rate;
        $journal->vat_amount        = $tax_invoice->vat_amount;
        $journal->total_amount      =  $tax_invoice->amount+$tax_invoice->vat_amount+$tax_invoice->discount_amount+$tax_invoice->total_toll_fee;
        ...
        if( $tax_invoice->total_toll_fee>0){
            $vat_ac_head= AccountHead::find(40); // vat receivable
            $jl_record= new JournalRecord();
            $jl_record->journal_id          = $journal->id;
            $jl_record->project_details_id  = $journal->project_id;
            $jl_record->cost_center_id      =  $journal->cost_center_id;
        ...
        $voucher_type="DR";
        if(  $journal->pay_mode   == 'Cash' ||  $journal->pay_mode   == 'Card'){
        ...
        if( $journal->pay_mode  =='Cash' ||  $journal->pay_mode  =='Card'){
            ...
            $journal= new Journal();
            $journal->transection_type  = "Customer Invoice";
            $journal->transaction_type  = "Receipt";
            $journal->without_gst       = $tax_invoice->amount;
            $journal->gst_subtotal      = $tax_invoice->vat_amount;
            $journal->project_id        = $tax_invoice->project_id;
            $journal->journal_no        = $journal_no;
            ...
            $journal->party_info_id     = $tax_invoice->customer_id;
            $journal->account_head_id   = 123;
            $journal->amount            =  $tax_invoice->paid_amount + $tax_invoice->discount_amount ;
            $journal->tax_rate          = 0;
            $journal->vat_amount        = 0;
            $journal->total_amount      =  $tax_invoice->paid_amount + $tax_invoice->discount_amount ;
            $journal->narration         = 'Transport service provided by '. $tax_invoice->pay_mode;
            $journal->created_by        = Auth::id();
            $journal->voucher_type      = 'default';
            $journal->authorized        = 1;
            ...
            {
                $ac_head_cr= AccountHead::find(29); // Accounts Receivable
                $jl_record= new JournalRecord();
                $jl_record->journal_id          = $journal->id;
                $jl_record->project_details_id  = $journal->project_id;
                $jl_record->cost_center_id      = $journal->cost_center_id;
            ...
            $ac_head_cr= AccountHead::find(3); // Accounts Receivable
            $jl_record= new JournalRecord();
            $jl_record->journal_id          = $journal->id;
            $jl_record->project_details_id  = $journal->project_id;
            $jl_record->cost_center_id      = $journal->cost_center_id;
            ...
            $jl_record->master_account_id   = $ac_head_cr->master_account_id;
            $jl_record->account_head        = $ac_head_cr->fld_ac_head;
            $jl_record->amount              = $tax_invoice->paid_amount+$tax_invoice->discount_amount;
            $jl_record->total_amount        = $tax_invoice->paid_amount+$tax_invoice->discount_amount;
            $jl_record->transaction_type    = 'CR';
            $jl_record->journal_date        = $journal->date;
            $jl_record->account_type_id     = $ac_head_cr->account_type_id;
            $jl_record->gst_amount          = $journal->total_amount;
        ...
        return redirect('customer-inv/invoice-approval-list')->with($notification);
       }
    }
    ...
    public function invoice_athurization_list(Request $request){
        Gate::authorize('app.invoice.invoice_authorize');
        $users = User::all();
        $invoices= TaxInvoiceTemp::where('status',"Authorize")->orderBy('created_at', 'DESC');
        if($request->user_id){
    ...
    public function invoice_approval_list(Request $request){
        Gate::authorize('app.invoice.invoice_approval');
        $users = User::all();
        $invoices= TaxInvoiceTemp::orderBy('id', 'desc')->where('status',"Approve");
        if($request->user_id){
            $invoices = $invoices->where('created_by', $request->user_id)
                        ->where('authorized_by', $request->user_id)
                        ->where('approved_by', $request->user_id)
    ...
    public function declined_invoice_list(Request $request){
        Gate::authorize('app.invoice.invoice_view');
        $users = User::all();
        $invoices= TaxInvoiceTemp::orderBy('id', 'desc')->where('status',"Decline");
        if($request->user_id){
            $invoices = $invoices->where('created_by', $request->user_id)
                        ->where('authorized_by', $request->user_id)
                        ->where('approved_by', $request->user_id)
    ...
    public function draft_invoice_list(Request $request){
        $invoices= TaxInvoiceTemp::where('status',"Draft")->orderby('id', 'desc')->get();
    ...
    public function draft_invoice_submit($id){
        ...
        return redirect('customer-inv/draft-invoice-list')->with($notification);
    }
    ...
    public function draft_invoice_print($id){
        ...
        $terms = PayTerm::get();
        return view('backend.customer-invoice.invoice', compact('invoice', 'terms'));
    }
    ...
    public function multiple_approval_invoice(Request $request){
        ...
        foreach($records as $key => $recode){
            ...
            $tax_invoice->amount        = $pre_inv->amount;
            $tax_invoice->discount_amount    = $pre_inv->discount_amount;
            $tax_invoice->vat_amount    = $pre_inv->total_vat_amount;
            $tax_invoice->total_toll_fee    = $pre_inv->total_toll_fee;
            $tax_invoice->invoice_type   = $pre_inv->invoice_type;
            ...
            // $journal->tax_rate          = $v_rate;
            // $journal->vat_amount        = $tax_invoice->vat_amount;
            // $journal->total_amount      = $tax_invoice->amount;
            // $journal->narration         = 'Transport service provided by '. $tax_invoice->pay_mode ;
            // $journal->created_by        = Auth::id();
            // $journal->voucher_type      = 'default';
            // $journal->authorized        =1;
            ...
            // // Main Entry
            // $acc_head= AccountHead::find(20); // servcie revenue account
            // $jl_record= new JournalRecord();
            // $jl_record->journal_id          = $journal->id;
            // $jl_record->project_details_id  = $journal->project_id ;
            // $jl_record->cost_center_id      = $journal->cost_center_id;
            ...
            // $dr_cr_voucher->narration       = 'Transport service provided by '.  $journal->pay_mode  ;
            ...
            //     $journal->party_info_id     = $tax_invoice->customer_id;
            //     $journal->account_head_id   = 123;
            //     $journal->amount            = $tax_invoice->paid_amount;
            //     $journal->tax_rate          = 0;
            //     $journal->vat_amount        = 0;
            //     $journal->total_amount      = $tax_invoice->paid_amount;
    ...
    public function truck_service_process(Request $request){
        ...
        //return $records;
        $vats = VatRate::all();
        return view('backend.customer-invoice.truck-service-add-summarry', compact('records','customer_id', 'date_to', 'date_from', 'truck_ids','vats'));
    }
