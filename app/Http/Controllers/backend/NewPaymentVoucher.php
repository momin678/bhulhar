...
class NewPaymentVoucher extends Controller
{
    public function payment_voucher_form(){
        ...
        $modes = PayMode::all();
        $invoices= SupplierInvoice::Where('due_amount','>',0)->get();
        // dd($invoices);

        return view('backend.new-payment-voucher.new-create', compact('projects', 'cCenters', 'pInfos','modes','invoices'));
    }
    ...
    public function payment_voucher_store(Request $request){
        ...
        if($request->voucher_type=='advance'){
            ...
            foreach($request->input('group-a') as $each_inv){
                ...
                $rv_details->payment_date   = $new_date;
    ...
    public function make_authorize($id){
        ...
        elseif($voucher->status=="Approve")
        {
            ...
            foreach($voucher->items as $temdet){
                ...
                $supplier_invoice = SupplierInvoice::find($temdet->invoice_id);
                ...
                $rv_details->save();
                $supplier_invoice->due_amount = $supplier_invoice->due_amount - $rv_details->paid_amount;
        ...
        $journal->transaction_type  = "Payment Voucher";
        ...
        $ac_head_dr= AccountHead::find(5); // payable account
