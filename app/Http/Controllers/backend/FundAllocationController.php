...
use App\Notifications\FundAllocationMessage;
use App\PayMode;
use App\User;
use Carbon\Carbon;
...
class FundAllocationController extends Controller
{
    ...
    public function index(Request $request)
    {
      //  return Auth::user()->notifications;
        $modes = PayMode::whereNotIn('id', [2,4,3])->get();
        // dd($modes);
        ...
        return view('backend.fund-allocation.index',compact('modes','allocations'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($this->available_pay_amount($request->from_account)>$r_amount){
            ...
            // $usersToNotify = User::whereHas('role.permissions', function ($query) {
            //     $query->where('slug', 'Admin_approve');
            // })->orWhereHas('addPermissions', function ($query) {
            //     $query->where('slug', 'Admin_approve');
            // })->get();

            // foreach ($usersToNotify  as $admin) {
            //     $admin->notify(new FundAllocationMessage($fund_allocation));
            // }
        ...
        if($fund){
            ...
            if($this->available_pay_amount($fund->account_id_from)>$r_amount){
                if($fund->approved==false) {
                    ...
                    $from=$fund->account_id_from==1?1:($fund->account_id_from==5?2:10);
                    $to=$fund->account_id_to==1?1:($fund->account_id_to==5?2:10);
                    ...
                    {
                        $transaction_fee = AccountHead::find(11);
                        $jl_record = new JournalRecord();
                        $jl_record->journal_id          = $journal->id;
                        $jl_record->project_details_id  = 1;
                        $jl_record->cost_center_id      = 1;
                    ...
                    return redirect('fund-allocation-approve')->with($notification);
    ...
    public function fund_allocation_approval2($id)
    {
        $fund=FundAllocation::find($id);
        if($fund)
        {
            ...
            if($this->available_pay_amount($fund->account_id_from)>$r_amount){
                if($fund->approved==false)
                {
                    ...
                    $from=$fund->account_id_from==1?1:($fund->account_id_from==4?2:93);
                    $to=$fund->account_id_to==1?1:($fund->account_id_to==4?2:93);

                    $from_head = AccountHead::find($from); // receivable account
                    $jl_record = new JournalRecord();
                    $jl_record->journal_id          = $journal->id;
                    $jl_record->project_details_id  = 1;
                    $jl_record->cost_center_id      = 1;
                    ...
                    $to_head = AccountHead::find($to); // receivable account
                    $jl_record = new JournalRecord();
                    $jl_record->journal_id          = $journal->id;
                    $jl_record->project_details_id  = 1;
                    $jl_record->cost_center_id      = 1;
                    ...
                    if($fund->transaction_cost>0)
                    {
                        $transaction_fee = AccountHead::find(92); // receivable account
                        $jl_record = new JournalRecord();
                        $jl_record->journal_id          = $journal->id;
                        $jl_record->project_details_id  = 1;
                        $jl_record->cost_center_id      = 1;
    ...
    public function fund_allocation_delete($id){
        $fund_collections = FundAllocation::find($id);
        $fund_collections->delete();
        $notification = array(
            'message'       => 'Fund Allocation Delete !',
            'alert-type'    => 'success'
        );
        return back()->with($notification);
    }
    public function fund_allocation_edit(Request $request){
        $fund = FundAllocation::find($request->id);
        // dd($fund);
        $modes = PayMode::whereNotIn('id', [2,4])->get();
        return view('backend.fund-allocation.edit', compact('fund', 'modes'));
    }
    public function multiple_approval_fund_allocation(Request $request){
        // dd($request->all());
        $records = $request->records;
        foreach($records as $record){
            $this->fund_allocation_approval($record);
        }
        $notification = array(
            'message'       => 'Approve Successfully!',
            'alert-type'    => 'success'
        );
        return redirect()->route('fund-allocation.index')->with($notification);
    }
}
