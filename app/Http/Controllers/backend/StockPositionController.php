...
class StockPositionController extends Controller
{
    public function stockPosition(Request $request)
    {
        ...
        return view('backend.stock.stockPosition', compact('Stock'));
    }
    ...
    public function product_purchase_sale_report(Request $request, $id){
        $product = Product::find($id);
        $sales = InvoiceItem::where('item_id', $id)->get();
        $purchases = PurchaseItem::where('product_id', $id)->get();
        return view('backend.stock.product-purchase-sales-reports', compact('sales','purchases', 'product'));
    }
    public function printStockPositionItem($products)
    {
        $product=Product::find($products);
        return view('backend.pdf.stockPositionProduct', compact('product'));
    }

    public function printStockPositionCat($category)
    {
        $category=Category::find($category);
        return view('backend.pdf.stockPositionCategory', compact('category'));
    }
}
