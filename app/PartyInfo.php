...
class PartyInfo extends Model
{
    ...
    {
        $check = DB::select("
        SELECT id AS id FROM tax_invoices WHERE customer_id = $this->id
        UNION
    ...
    public function due_amount(){
        $purchase_due = PurchaseExpense::where('party_id', $this->id)->where('due_amount', '>', 0)->sum('due_amount');
        $supplier_due = SupplierInvoice::where('supplier_id', $this->id)->where('due_amount', '>', 0)->sum('due_amount');
        return $supplier_due+$purchase_due;
    }
}
